    ...
    for i in range(iterations):
        ...
        for x in range(0,int(nodes[0][-1]),1):
            if len((list_node_final[x]))==2:
                Temp_new[x]=(1-(4*fourier))+(fourier*((Temp[int(list_node_final[x][0])-1])+(Temp[int(list_node_final[x][1])-1])))
                for load in loads:
                ...
                Temp_new[x]=(1-(4*fourier))+(fourier*((Temp[int(list_node_final[x][0])-1])+(Temp[int(list_node_final[x][1])-1])+
                                                      (Temp[int(list_node_final[x][2])-1])+(Temp[int(list_node_final[x][3])-1])))
                for load in loads:
                    Temp_new[int(load)-1]=100
                Temp = Temp_new
...
def iteration1(d_xy, alpha, delta_t, nodes, loads, iterations, list_node_final):    
    fourier = alpha * delta_t / (d_xy ** 2)     
